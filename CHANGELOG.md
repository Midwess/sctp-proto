...
# 0.10.7
  ...
  * **Raise `for_relay()` `max_cwnd_bytes` 250 KB -> 800 KB.** With turn-rs `0e114a3` (production / main) bounding kernel UDP `SO_SNDBUF` to 64 KB on the relay listener, the multi-second delivery jitter that drove the cwnd cap down through 400 KB -> 250 KB across 0.10.4-0.10.6 should be eliminated at the source. The cap can now serve its intended role -- a sanity ceiling against runaway cwnd growth on misbehaving paths -- rather than being the throttle that ratelimits healthy paths.
  * Per-slot ceiling at 150 ms min_rtt: 800 KB / 0.150 s = ~42 Mbps. Aggregate across 3 slots: ~125 Mbps. Stays well clear of 0.9.3's resonance point at exactly 500 KB BDP for the prior path profile, and even further from the 1.875 MB BDP at 100 Mbps × 150 ms RTT (so we don't park-at-BDP).
  * Pair-deploy with turn-rs `0e114a3`. If the TURN host is still on the old turn-rs (8 MB SO_SNDBUF default), expect bufferbloat to return at 800 KB cap -- regress to 0.10.6 in that case.
  * 213/213 tests green. config-only change.
...
# 0.10.5
  ...
  * **Bump `for_relay()` `max_cwnd_bytes` 250 KB → 400 KB.** The 250 KB cap was below path BDP for higher-RTT paths (Vietnam ↔ Asia relay observed at 150ms min_rtt, BDP ~500-900 KB). Healthy slots in the 06:05 trace parked at 250 KB cap (jit_p95 at 1.5-6 ms, zero RACK marks) and stalled at ~12 Mbps per slot when ~25 Mbps was achievable. 400 KB stays clear of the 0.9.3 resonance point at exactly 500 KB while restoring throughput on high-RTT paths. With sticky historical-max + adaptive RACK now in place (the missing pieces 0.9.3 didn't have), brief parking near BDP shouldn't resonate -- marks-on-jitter trigger floor lift instead of cwnd cut.
  * 213/213 tests green.

# 0.10.4

  * **Bufferbloat brake on `for_relay()`**: `with_max_cwnd_bytes(Some(250_000))`. Production trace at `2026-04-27T05:47` revealed a new failure mode the 0.10.3 sticky-historical-max design could not handle: paths whose tail grows under load (TURN-egress queue depth scaling with sender cwnd). Slot c37f00b5's `reo_jit_p95_us` ratcheted `1224 -> 1647 -> 2089 -> 2728us` ms in successive bursts, with `reo_wnd_us` lifting `1592 -> 2053 -> 2718 -> 2728ms`. Every adaptive lift just enabled more sending, deepening the queue, producing larger marks, lifting the floor again. This is **congestion masquerading as jitter** -- RACK can't distinguish them from delivery deltas alone. The cwnd cap at 250 KB hard-limits queue depth at the cause: even when the adaptive RACK floor permits a larger window, the cap holds. 250 KB sits below the BDP-resonance point that bit 0.9.3's 500 KB cap, while leaving headroom above the ~150-200 KB cwnd peaks 0.9.5 saw on healthy relay paths.
  * **Bound historical-max contribution at 2× static floor.** The 0.10.3 sticky historical-max prevented the decay-then-reburst whiplash, but on a bufferbloat path it became the runaway itself: each new burst exceeded the previous max, the tracker remembered it, and the floor climbed indefinitely toward `rto_min - 100ms`. Now `effective_rack_reo_wnd_floor` clamps the historical observed peak at `2 × static_floor` before the `/2` division, so historical contribution is at most `static_floor` (1200ms for `for_relay()`). The cwnd cap takes over for everything beyond.
  * **Revert `for_relay()` `rto_min_ms` 5000 -> 3000.** With the historical-max bounded and the cwnd cap in place, adaptive can no longer climb past 2400ms anyway; the 4900ms headroom granted by `rto_min=5000` is no longer needed. Restoring `rto_min=3000` gives back 2 seconds of T3-rtx safety on rare RACK-misses.
  * The TURN-side fix is **still required** for production health -- a relay producing 3+ second delivery jitter under load is overloaded. These changes let the SCTP layer limp along without catastrophic cwnd collapse, but the operational fix is whatever is wrong with `64.118.143.13/14:3478`.
  * 213/213 tests green.

# 0.10.3

  * **Sticky historical-max floor** in `JitterTracker`. The 30-second sliding window evicts hot samples during quiet periods, so after a jitter burst subsides the dynamic floor was decaying back to the static value -- and the next burst paid the full warmup penalty all over again (cwnd ground down before adaptive could re-lock). Production trace at `2026-04-27T05:26` showed slot `17fd7e7f` cycling: cwnd ramp to 2.8 MB, RACK marks at delta 1200-2030ms drove cwnd through 1.4 MB -> 718 KB -> 359 KB while adaptive lifted floor 1200 -> 1561 -> 2031ms; once samples decayed the floor returned to 1200ms static and the next burst started the same cycle.
    The fix: `JitterTracker` now records a per-association `historical_max_p95`. The effective floor becomes `max(1.3 * p95, 0.5 * historical_max_p95, static_floor)`, clamped below `rto_min - 100ms`. Once the tracker has observed e.g. p95=2000ms, the dynamic floor never falls below 1000ms again until the path changes (`maybe_reset_on_path_change` clears the historical max along with the sample window when `rack_min_rtt` shifts >50%).
  * **Bump `for_relay()` `rto_min_ms` 3000 -> 5000** to widen the adaptive headroom from 2900ms to 4900ms. Production envelope grew to 2030ms in the same trace, with the dynamic floor pinned at the old 2900ms cap. With rto_min=5000 adaptive can lift through 4900ms before the T3-rtx safety net engages. Cost: T3-rtx waits 5s instead of 3s on rare RACK-misses; acceptable on relay paths where the prior failure mode was T3 racing RACK.
  * No public API changes. 213/213 tests green (209 prior + 3 jitter_tracker historical-max + 1 effective-floor decay-survival).
